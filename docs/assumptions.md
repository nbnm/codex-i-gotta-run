...
6. `listen` is a message-first operator view: it reads recent thread history to print actual user and assistant messages before continuing with newly detected messages, and it marks the existing snapshot as seen so older backlog messages are not replayed again after resume.
